...
async function onRequestPost(context2) {
  ...
  if (env2.TURNSTILE_SECRET_KEY) {
    const passed = await verifyTurnstile(turnstileToken, env2.TURNSTILE_SECRET_KEY, request);
    if (!passed) {
  ...
  if (!env2.RESEND_API_KEY || !env2.FROM_ADDRESS) {
...
// ../.wrangler/tmp/pages-IP3La5/functionsRoutes-0.5341903681928168.mjs
var routes = [
  {
